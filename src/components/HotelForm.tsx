...
import { CheckCircle, ArrowRight, Send, Phone, Hotel } from "lucide-react";
...
export default function HotelForm({ className, defaultCity = "" }: HotelFormProps) {
  const [step, setStep] = useState(1);
  const [formData, setFormData] = useState({
    name: "",
    whatsapp: "",
    email: "",
    hotelCity: defaultCity,
    checkIn: "",
    checkOut: "",
    guests: "2",
    message: "",
  ...
  if (isSubmitted) {
    return (
      <div className={cn("bg-white p-8 rounded-[2rem] text-center border border-brand-gold/10 shadow-2xl w-full mx-auto h-[400px] flex flex-col justify-center", className)}>
        ...
        <p className="text-brand-blue/50 text-sm mb-8">JazakAllah Khair! We have received your hotel request. Our agent will contact you on WhatsApp and Email shortly.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <AnimatePresence mode="wait">
          {step === 1 && (
            ...
            >
               <div>
                  <h3 className="text-xl font-black text-brand-blue mb-1">Hotel Booking</h3>
                  <p className="text-brand-blue/40 text-xs mb-6 font-medium">Find the best stays for your trip.</p>
               </div>
               <div>
          ...
          {step === 2 && (
            ...
            >
               ...
               <div>
                  ...
                  <input type="number" min="1" className={inputClass} placeholder="2" value={formData.guests} onChange={(e) => update("guests", e.target.value)} />
               </div>
