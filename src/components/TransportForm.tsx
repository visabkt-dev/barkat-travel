...
import { CheckCircle, ArrowRight, Send, Phone, MapPin, Users, Calendar } from "lucide-react";
...
export default function TransportForm({ className, defaultRoute = "" }: TransportFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    email: "",
    pickup: "Doha, Qatar",
    dropoff: defaultRoute || "Makkah, Saudi Arabia",
    date: "",
    passengers: "1-4 Pax (Camry)",
    message: "",
  });
    ...
    try {
      ...
      const response = await fetch('/api/send-lead', {
        ...
        body: JSON.stringify({
          ...
          service_type: "GCC Transport",
          details: `Pickup: ${formData.pickup}\nDropoff: ${formData.dropoff}\nDate: ${formData.date}\nVehicle/Pax: ${formData.passengers}\nMessage: ${formData.message}`,
  ...
  if (isSubmitted) {
    return (
      <div className={cn("bg-white p-8 rounded-[2rem] text-center border border-brand-gold/10 shadow-2xl w-full mx-auto h-[400px] flex flex-col justify-center", className)}>
        ...
        <p className="text-brand-blue/50 text-sm mb-8">JazakAllah Khair! We have received your transport request. Our agent will contact you on WhatsApp and Email shortly.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <AnimatePresence mode="wait">
          {step === 1 && (
            ...
            >
               <div>
                  <h3 className="text-xl font-black text-brand-blue mb-1">Transport Quote</h3>
                  <p className="text-brand-blue/40 text-xs mb-6 font-medium">Safe & luxury cross-border travel.</p>
               </div>
               <div>
            ...
            >
               ...
               <div className="grid grid-cols-2 gap-3">
                  <div>
                      ...
                      <div className="relative">
                        <input type="date" className={cn(inputClass, "pl-10")} style={{colorScheme: 'light'}} value={formData.date} onChange={(e) => update("date", e.target.value)} />
                        <Calendar className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-gold pointer-events-none" />
                      </div>
                  </div>
                  <div>
                      <label className={labelClass}>Vehicle</label>
                      <div className="relative">
                        <select className={cn(inputClass, "pl-10")} value={formData.passengers} onChange={(e) => update("passengers", e.target.value)}>
                        ...
                        <Users className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-gold pointer-events-none" />
                      </div>
                  </div>
               </div>
          ...
          {step === 3 && (
            ...
            >
               ...
               <div>
                  ...
                  <textarea rows={2} placeholder="e.g. Need child seat, extra luggage..." className={cn(inputClass, "resize-none")}
